    ...
    return (
        <div className="flex-1 space-y-2 text-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center">
                <h3 className="text-xl font-semibold text-slate-800"> {store.name || 'Unknown Store'} </h3>
            ...
            {store.createdAt && (
                <p className="text-slate-700 mt-5">Applied on <span className="text-xs">{new Date(store.createdAt).toLocaleDateString()}</span></p>
            )}
            {store.user && (
